...
def read_data(spark):
    ...
    spark.sql('SELECT Count(Distinct(user_id)) FROM ss_df').show() # for 5% data: 138,040; for 1% data: 10,482; for 10% data:266,086
    ...
    subset_users = spark.sql("SELECT user_id FROM (SELECT Distinct(ss_df.user_id) FROM ss_df) ORDER BY RAND() LIMIT 55216") # made a limit for 40% of users
    ...
    half_users = spark.sql('SELECT DISTINCT(holdout_df.user_id) FROM holdout_df ORDER BY RAND() LIMIT 27608')
