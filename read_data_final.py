...
def read_data(spark):
    ...
    interactions = interactions.sample(False, 0.1, seed=1)
    ...
    # n = spark.sql('SELECT Count(Distinct(user_id)) FROM ss_df')
    # n.show() # 424806

    # Make a folder in dumbo hdfs -recom and save this file in parquet format
    ss_df.write.mode('overwrite').parquet("recom/ss_df.parquet")
    print("Subset saved")

    # read the file 
    # ss_df = spark.read.parquet("recom/ss_df.parquet")
    # ss_df.createOrReplaceTempView('ss_df') 

    # making validation and test sets using 40% data on the new set - 169,924

    subset_users = spark.sql("SELECT user_id FROM (SELECT Distinct(ss_df.user_id) FROM ss_df) ORDER BY RAND() LIMIT 169924") # made a limit for 40% of users
    ...
    half_users = spark.sql('SELECT DISTINCT(holdout_df.user_id) FROM holdout_df ORDER BY RAND() LIMIT 84962')
    ...
    val_df.write.mode('overwrite').parquet('recom/val_ss.parquet')
    print("Validation set saved in 'recom' folder")
    test_df.write.mode('overwrite').parquet('recom/test_ss.parquet')
    print("Test set saved in 'recom' folder")
    train_df.write.mode('overwrite').parquet('recom/train_ss.parquet')
